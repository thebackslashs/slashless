...
pub async fn handle_transaction(
    ...
    handle_transaction_internal(State(pool), Json(body), encoding_enabled).await
}
...
pub async fn handle_transaction_internal(
    ...
    // Execute MULTI
    let multi_result: Result<String, redis::RedisError> = redis::cmd("MULTI")
    ...
    match multi_result {
        Ok(_) => {
            ...
            // Execute EXEC
            let exec_result: Result<Vec<redis::Value>, redis::RedisError> = redis::cmd("EXEC")
                .query_async(&mut conn)
                .await;
